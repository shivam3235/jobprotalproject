...
def jsreg(request):
    ...
    contcactno=request.POST['contactno']
    email=request.POST['email']
    dob=request.POST['dob']
    qualfication=request.POST['qualfication']
    ...
    js=JobSeeker(name=name,gender=gender,address=address,contactno=contcactno,email=email,dob=dob,
                 qualfication=qualfication,exprience=exprience,keyskills=keyskills,regdate=regdate)

    log=Login(userid=email,password=password,usertype=usertype)
...
def ereg(request):
    ...
    emp=Employer(firmname=firmname,firmwork=firmwork,firmaddress=firmaddress,cpname=cpname,
                  cpcontactno=cpcontactno,cpemailaddress=cpemailaddress
                 ,aadharno=aadharno,panno=panno,gstno=gstno,regdate=regdate)
...
def enq(request):
    ...
    e=Enquiry(name=name,gender=gender,address=address,contactno=contactno,emailaddress=emailaddress,enquirytext=enquirytext
              ,posteddate=posteddate)
    e.save()

    msg = 'Enquiry is Submited Successfully'
...
def empchnagepassword(request):
    try:
        if request.session['employer']:
            return  render(request,'empchnagepassword.html')
    except:
        return render(request,'login.html')
...
def pjob(request):
    ...
    j=Jobs(firmname=firmname,jobtitle=jobtitle,post=post,jobdesc=jobdesc,qualification=qualification
           ,experience=experience,location=location,salarypa=salarypa,posteddate=posteddate,emailaddress=emailaddress)

    j.save()


    msg = 'Post job is succesfully'
...
def empchangepwd(request):
    ...
    if newpassword!=confirmpassword:
        msg=msg+'New password and confirm password are not equal'
        return render(request,'empchnagepassword.html',{'msg':msg})
    ...
    try:
        ...
        log.save()
        mm='password change successfuly please login again'
        return render(request,'login.html',{'mm':mm})
    except:
        msg=msg+'Old paswword is not match'
    return render(request,'empchnagepassword.html',{'msg':msg})
...
def jobchangepwd(request):
    ...
    msg='Message='
    if newpassword!=confirmpassword:
        msg=msg+'Newpassword and Confirm Password are not Equal'
        return render(request,'jobchangepassword.html',{'msg':msg})

    userid=request.session['jobseeker']
    try:
        kk='Password Change Successfully Please Login Again'
        ...
        return render(request,'login.html',{'kk':kk})
    except:
...
def appliedjobs(request,id):
    ...
    obj2=JobSeeker.objects.get(email=request.session['jobseeker'])
    name=obj2.name
    gender=obj2.gender
    address=obj2.address
    contactno=obj2.contactno
    emailaddress=obj2.email
    dob=obj2.dob
    qualfication=obj2.qualfication
    exprience=obj2.exprience
    keyskills=obj2.keyskills
    applieddate=datetime.datetime.today()

    aj=AppliedJobs(empemailaddress=empemailaddress,jobtitle=jobtitle,post=post,gender=gender
                   ,name=name,contactno=contactno,emailaddress=emailaddress,address=address,dob=dob
                   ,qualfication=qualfication,exprience=exprience,keyskills=keyskills,applieddate=applieddate)
...
def enquiries(request):
    try:
        if request.session['admin']:
            ...
            return render(request,'enquiries.html',{'en':en})
